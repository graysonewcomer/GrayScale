...
## Bugs
- ~~**Apply Edit dumps you back on the first game tab.**~~ — fixed July 2026.
  The progress poll's `done` branch no longer calls
  `window.location.reload()`; it patches the affected card in place via a new
  shared `window.GrayScale.applyReplace` helper (re-keys the id, rewrites the
  size, cache-busts the thumbnail with `?v=`) and closes the editor, keeping
  the current game tab and scroll position on the clip you just edited.
